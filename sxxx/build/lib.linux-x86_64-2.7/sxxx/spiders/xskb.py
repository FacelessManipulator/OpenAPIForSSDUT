...
class XskbSpider(scrapy.Spider):
    ...
    def start_requests(self):
        if hasattr(self, 'account') and hasattr(self, 'password'):
            self.fetch_list = [{'account':self.account, 'password': self.password}]
        while hasattr(self, "fetch_list") and len(self.fetch_list) > 0:
            try:
